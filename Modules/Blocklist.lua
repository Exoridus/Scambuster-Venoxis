local AddonName, Addon = ...;
local AceLocale = LibStub("AceLocale-3.0");
local Utils = Addon:GetModule("Utils") --[[@as Utils]];
---@class Blocklist : AceModule
local Blocklist = Addon:NewModule("Blocklist");
---@type AddonLocale
local L = AceLocale:GetLocale(AddonName);
local sort, strlower = sort, strlower;

function Blocklist.SortComparator(a, b)
  local nameA = a and (a.players and a.players[1] or a).name;
  local nameB = b and (b.players and b.players[1] or b).name;

  if nameA and nameB then
    return strlower(nameA) < strlower(nameB);
  end

  return not nameA;
end

function Blocklist:OnInitialize()
  self:Sort();
end

function Blocklist:Sort()
  for _, entry in ipairs(self.Entries) do
    if entry and entry.players and #entry.players > 0 then
      sort(entry.players, self.SortComparator);
    end
  end

  sort(self.Entries, self.SortComparator);
end

function Blocklist:GetUniquePlayers()
  local players = {};

  for _, entry in ipairs(self.Entries) do
    if entry.players and #entry.players > 0 then
      for _, player in ipairs(entry.players) do
        if not players[player.guid] then
          players[player.guid] = {
            guid = player.guid,
            name = player.name,
            class = player.class,
            faction = player.faction,
          };
        elseif players[player.guid].name ~= player.name then
          Utils:PrintWarning(L.FOUND_NAME_COLLISION, player.guid, players[player.guid].name, player.name);
        end
      end
    elseif entry.name then
      if not players[entry.guid] then
        players[entry.guid] = {
          guid = entry.guid,
          name = entry.name,
          class = entry.class,
          faction = entry.faction,
        };
      elseif players[entry.guid].name ~= entry.name then
        Utils:PrintWarning(L.FOUND_NAME_COLLISION, entry.guid, players[entry.guid].name, entry.name);
      end
    end
  end

  return GetValuesArray(players);
end

function Blocklist:GetUniquePlayersByPredicate(FilterPredicate)
  return tFilter(self:GetUniquePlayers(), FilterPredicate, true);
end

Blocklist.Entries = {
  [1] = {
    name = "Alcyona",
    guid = "Player-4477-03C5CE1A",
    class = "PALADIN",
    faction = "Horde",
    description = "Booster kickt Leute / geht Offline nach Bezahlung ohne Leistung",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/984066550950662194",
    category = "trade",
    level = 3,
  },
  [2] = {
    name = "Alonewoolf",
    guid = "Player-4477-045EA655",
    class = "DRUID",
    faction = "Horde",
    description = "Stellt im 5er vor dem Endboss den PM ein und Ninjat fast alles / bisher 3 Meldungen zu diesem Spieler",
    url = "https://discord.com/channels/613060619738021890/1029711210918191154/1073129858361737276",
    category = "dungeon",
    level = 3,
  },
  [3] = {
    name = "Amaluca",
    guid = "Player-4477-03AEE3AE",
    class = "PRIEST",
    faction = "Horde",
    description = "Vergibt zuerst sein Item falsch und behält sich dann einen Token aus Frust ein, der ihm nicht gehört",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1086865128026275930",
    category = "raid",
    level = 3,
  },
  [4] = {
    name = "Amnorth",
    guid = "Player-4477-0522EA50",
    class = "PALADIN",
    faction = "Horde",
    description = "ICC 2523 GDKP Scammer // Geht mit dem gesamten Pot 180k Gold offline / VORSICHT GDKP SCAMMER! Hat sich bereits umbenannt	",
    url = "https://discord.com/channels/613060619738021890/1029711210918191154/1123313827539329044",
    category = "dungeon",
    level = 3,
  },
  [5] = {
    name = "Arbeitslos",
    guid = "Player-4477-01B8DFEE",
    class = "ROGUE",
    faction = "Horde",
    description = "Bekommt vom RL ein Item falsch zugewiesen und geht nach der Aufforderung dieses zu traden direkt offline",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1078668551566671962",
    category = "trade",
    level = 3,
  },
  [6] = {
    name = "Arcturuz",
    guid = "Player-4477-03B6C66D",
    class = "SHAMAN",
    faction = "Horde",
    description = "Bekommt eine Waffe unabsichtlich falsch getradet und geht sofort damit offline",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1042597576882528296",
    category = "trade",
    level = 3,
  },
  [7] = {
    name = "Asoria",
    guid = "Player-4477-0336EE6B",
    class = "PALADIN",
    faction = "Horde",
    description = "PDK 25 HC / verteilt Loot nicht nach höchstem roll sondern nach eigenem ermessen random und scamt damit Items.",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1162911211864723496",
    category = "raid",
    level = 3,
  },
  [8] = {
    name = "Attakin",
    guid = "Player-4477-0459E2E3",
    class = "ROGUE",
    faction = "Horde",
    description = "Ulduar 10er / Kickt Spieler grundlos aus der Gruppe und scammt damit deren ID und den Loot",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1167004968365084684",
    category = "raid",
    level = 3,
  },
  [9] = {
    description = "Plündert gern mal die Gildenbank",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1094685096780513331",
    category = "trade",
    level = 3,
    players = {
      [1] = {
        name = "Autoritä",
        guid = "Player-4477-049EDE13",
        class = "ROGUE",
        faction = "Horde",
      },
      [2] = {
        name = "Jondeepfreez",
        guid = "Player-4477-0464DE81",
        class = "MAGE",
        faction = "Horde",
      },
    },
  },
  [10] = {
    name = "Baerentatzee",
    guid = "Player-4477-05111D95",
    class = "PALADIN",
    faction = "Horde",
    description = "Öffnet einen MS-OS Raid und vergibt dann eine Waffe die eigentlich BIS für jemanden ist, an jemand anderen für OS. Nach Konfrontation zeigt er keine Reue und beteuert nicht alles wissen zu können.",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1148757997665923073",
    category = "raid",
    level = 3,
  },
  [11] = {
    description = "AK 25 / Specrun / Item wird entgegen der Lootregeln ungerechtfertigt nicht verteilt / Vorsicht: Scamgefahr in diesen Raids	",
    url = "https://discord.com/channels/613060619738021890/972036752283926578/1164499989511295016",
    category = "raid",
    level = 3,
    players = {
      [1] = {
        name = "Bimbambärchi",
        guid = "Player-4477-02FE3F47",
        class = "DRUID",
        faction = "Horde",
      },
      [2] = {
        name = "Dawner",
        guid = "Player-4477-0212BE82",
        class = "WARLOCK",
        faction = "Horde",
      },
      [3] = {
        name = "Derwan",
        guid = "Player-4477-0113DD6F",
        class = "MAGE",
        faction = "Horde",
      },
      [4] = {
        name = "Morde",
        guid = "Player-4477-020644CC",
        class = "WARRIOR",
        faction = "Horde",
      },
    },
  },
  [12] = {
    description = "Scamt Prio1 Item und vergibt es an ein Gildenmitglied",
    url = "https://discord.com/channels/613060619738021890/972036752283926578/1062805295056818266",
    category = "raid",
    level = 3,
    players = {
      [1] = {
        name = "Boomboompøw",
        guid = "Player-4477-04C424C5",
        class = "HUNTER",
        faction = "Horde",
      },
      [2] = {
        name = "Nøxic",
        guid = "Player-4477-03355A2D",
        class = "PRIEST",
        faction = "Horde",
      },
    },
  },
  [13] = {
    name = "Brodyrich",
    guid = "Player-4477-04CA7FA9",
    class = "DEATHKNIGHT",
    faction = "Horde",
    description = "Geht mit 75k vom Gdkp offline und kickt alle vom DC-Server / Dc-Tag = Gigawizard#8360",
    url = "https://discord.com/channels/613060619738021890/972036752283926578/1059169678292942878",
    category = "gdkp",
    level = 3,
  },
  [14] = {
    name = "Buik",
    guid = "Player-4477-0519A01C",
    class = "SHAMAN",
    faction = "Horde",
    description = "PDK 25er / geht mit dem gesamten Pot 200k Gold offline / spricht nur Englisch",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1154039081169330286",
    category = "gdkp",
    level = 3,
    aliases = { "Gladbach" },
  },
  [15] = {
    name = "Builder",
    guid = "Player-4477-037D513B",
    class = "ROGUE",
    faction = "Horde",
    description = "Würfelt in HC+ 5 Mann auf alles Bedarf weil er behauptet verzauberer zu sein und es für Splitter dissen will",
    url = "https://discord.com/channels/613060619738021890/1029711210918191154/1077230883871924325",
    category = "dungeon",
    level = 3,
  },
  [16] = {
    description = "Plündert gern mal die Gildenbank",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1036332378873942187",
    category = "trade",
    level = 3,
    players = {
      [1] = {
        name = "Buyvet",
        guid = "Player-4477-03B5135F",
        class = "PALADIN",
        faction = "Horde",
      },
      [2] = {
        name = "Dejox",
        guid = "Player-4477-043A6C38",
        class = "ROGUE",
        faction = "Horde",
      },
      [3] = {
        name = "Dkgoesbrbr",
        guid = "Player-4477-047A2CE0",
        class = "DEATHKNIGHT",
        faction = "Horde",
      },
      [4] = {
        name = "Itsmebiatch",
        guid = "Player-4477-03B19976",
        class = "MAGE",
        faction = "Horde",
        aliases = { "Ungeiimpfter" },
      },
    },
  },
  [17] = {
    description = "ICC25 Prio3 run / Erhält ungerechtfertigt ein Item und gibt dieses nicht heraus (in Absprache mit dem Raidleiter) / Vorsicht: Scamgefahr",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1167579393640636426",
    category = "raid",
    level = 3,
    players = {
      [1] = {
        name = "Carlclunky",
        guid = "Player-4477-04DD9430",
        class = "WARRIOR",
        faction = "Horde",
      },
      [2] = {
        name = "Robzero",
        guid = "Player-4477-038C44A7",
        class = "PALADIN",
        faction = "Horde",
      },
    },
  },
  [18] = {
    description = "Steckt sich nach PrioRun den Drachen und die Beutetasche aus Obsi 3D 10er ein obwohl er es nicht auf Prio hatte",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1084141800903684136",
    category = "raid",
    level = 3,
    players = {
      [1] = {
        name = "Convexyz",
        guid = "Player-4477-04F4059E",
        class = "ROGUE",
        faction = "Horde",
      },
      [2] = {
        name = "Xevnoc",
        guid = "Player-4477-04D80016",
        class = "PRIEST",
        faction = "Horde",
      },
    },
  },
  [19] = {
    name = "Cradran",
    guid = "Player-4477-04F82F8F",
    class = "WARLOCK",
    faction = "Horde",
    description = "Rollt gern ohne zu Zahlen",
    url = "https://docs.google.com/spreadsheets/d/1IKAr8A4P0-LhkXqMxizvgYy1E2gph_00M_O0r3rDGkY/edit",
    category = "trade",
    level = 3,
  },
  [20] = {
    name = "Cruzitô",
    guid = "Player-4477-044B93D3",
    class = "MAGE",
    faction = "Horde",
    description = "Steckt gern Items sich selbst oder seinen Kumpels zu. ",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1044684405840564316",
    category = "trade",
    level = 3,
  },
  [21] = {
    name = "Cycral",
    guid = "Player-4477-04E27973",
    class = "PALADIN",
    faction = "Horde",
    description = "Vergibt in Naxx10er ein Endboss item an einen Spieler der es nicht gewonnen hat und der dann instant offline geht",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1096600239164629114",
    category = "raid",
    level = 3,
  },
  [22] = {
    name = "Cînderrella",
    guid = "Player-4477-046669DD",
    class = "MAGE",
    faction = "Horde",
    description = "GDKP - geht mit dem gesamten Pot offline / 81.402 Gold / Zweiter Gdkp Scam: 56.500",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1080868571279790210",
    category = "gdkp",
    level = 3,
    aliases = { "Pumperlumper", "Dannymage" },
  },
  [23] = {
    name = "Dakiny",
    guid = "Player-4477-042FF318",
    class = "WARRIOR",
    faction = "Alliance",
    description = "Kara Mount geninjat + leave",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/984075667069095936",
    category = "raid",
    level = 3,
  },
  [24] = {
    name = "Dancus",
    guid = "Player-4477-033E14B7",
    class = "DRUID",
    faction = "Horde",
    description = "Würfelt gern um Gold und zahlt dann nicht",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1135607685920858162",
    category = "trade",
    level = 3,
  },
  [25] = {
    description = "Danser und Intonraiha ninjan sich gern mal gegenseitig Items zu als Plündermeister. Scam Pugs!",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1046099363648245930",
    category = "raid",
    level = 3,
    players = {
      [1] = {
        name = "Danser",
        guid = "Player-4477-043160B7",
        class = "ROGUE",
        faction = "Horde",
      },
      [2] = {
        name = "Intonraiha",
        guid = "Player-4477-04316087",
        class = "PALADIN",
        faction = "Horde",
      },
    },
  },
  [26] = {
    description = "Geheime Absprachen bezüglich Gold-Gebote in seinen Gdkp-Raids",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1054029597320163408",
    category = "gdkp",
    level = 3,
    players = {
      [1] = {
        name = "Daserus",
        guid = "Player-4477-04AC5F5C",
        class = "PALADIN",
        faction = "Horde",
      },
      [2] = {
        name = "Fleta",
        guid = "Player-4477-029C4B7E",
        class = "WARRIOR",
        faction = "Horde",
      },
    },
  },
  [27] = {
    name = "Deinorc",
    guid = "Player-4477-031A85E5",
    class = "WARRIOR",
    faction = "Horde",
    description = "Ninjat durch PM-Bug das Prio1 Item eines anderen Spielers und geht instant offline",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1064114144883785838",
    category = "raid",
    level = 3,
  },
  [28] = {
    name = "Diggernîck",
    guid = "Player-4477-02557182",
    class = "DRUID",
    faction = "Horde",
    description = "Rollt gern ohne zu Zahlen",
    url = "https://discord.com/channels/613060619738021890/972036752283926578/976906725380722688",
    category = "trade",
    level = 3,
  },
  [29] = {
    name = "Dirtyrouge",
    guid = "Player-4477-03AE5C6F",
    class = "ROGUE",
    faction = "Horde",
    description = "Lootdrama / spricht kein Deutsch / versucht zu scammen",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/960870173500399618",
    category = "gdkp",
    level = 3,
  },
  [30] = {
    name = "Dreamyx",
    guid = "Player-4477-03B0FD91",
    class = "SHAMAN",
    faction = "Horde",
    description = "	ZA GDKP Geht mit dem gesamten Pot offline / Will loot behalten statt diss aber nicht zahlen.",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/972439422299693076",
    category = "gdkp",
    level = 3,
  },
  [31] = {
    name = "Elfinmage",
    guid = "Player-4477-0487B9B5",
    class = "MAGE",
    faction = "Horde",
    description = "PDK 25er / Geht mit dem gesamten Pot offline 96.500 Gold / spricht nur Englisch",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1157751428325052498",
    category = "gdkp",
    level = 3,
    aliases = { "Mentolizar" },
  },
  [32] = {
    name = "Elrether",
    guid = "Player-4477-044348A4",
    class = "MAGE",
    faction = "Horde",
    description = "Booster kickt Leute / geht Offline nach Bezahlung ohne Leistung",
    url = "https://docs.google.com/spreadsheets/d/1IKAr8A4P0-LhkXqMxizvgYy1E2gph_00M_O0r3rDGkY/edit",
    category = "dungeon",
    level = 3,
  },
  [33] = {
    name = "Faktorial",
    guid = "Player-4477-034B6F50",
    class = "MAGE",
    faction = "Horde",
    description = "Kassiert 120 Gold - pullt - stirbt und geht dann offline",
    url = "https://docs.google.com/spreadsheets/d/1IKAr8A4P0-LhkXqMxizvgYy1E2gph_00M_O0r3rDGkY/edit",
    category = "dungeon",
    level = 3,
  },
  [34] = {
    name = "Fikky",
    guid = "Player-4477-03BD37F7",
    class = "MAGE",
    faction = "Horde",
    description = "Blacklisted sich selbst: Verkauft ZA Bär für 2K und fügt es nicht dem GDKP Pot hinzu.",
    url = "https://discord.com/channels/613060619738021890/972036752283926578/1009841223105785947",
    category = "gdkp",
    level = 3,
  },
  [35] = {
    name = "Fyfy",
    guid = "Player-4477-04983B8E",
    class = "DRUID",
    faction = "Horde",
    description = "	Needet alle Items in 5mann HC+ / mehrfach reportet",
    url = "https://discord.com/channels/613060619738021890/1029711210918191154/1065655958795194378",
    category = "dungeon",
    level = 3,
  },
  [36] = {
    name = "Gladix",
    guid = "Player-4477-04E405A6",
    class = "ROGUE",
    faction = "Horde",
    description = "Ninjat in Archavons Kammer 25 zwei Items",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1093498181402251294",
    category = "raid",
    level = 3,
  },
  [37] = {
    name = "Gladix",
    guid = "Player-4477-04E405A6",
    class = "ROGUE",
    faction = "Horde",
    description = "Plündert am 28.06.2023 eine Gildenbank",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1124450377534800052",
    category = "trade",
    level = 3,
  },
  [38] = {
    name = "Glurenz",
    guid = "Player-4477-03DA297A",
    class = "WARRIOR",
    faction = "Horde",
    description = "Kickt gern ungerechtfertigt Spieler aus HC ++ Dungeons und klaut ihnen somit die ID",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1156349549238030417",
    category = "gdkp",
    level = 3,
    aliases = { "Popiat" },
  },
  [39] = {
    description = "Kassiert beim Specrun Items die ihm nicht zustehen von seinem Raidleiter Kumpel und geht damit offline.",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1152618912752619610",
    category = "raid",
    level = 3,
    players = {
      [1] = {
        name = "Gravehunt",
        guid = "Player-4477-03524E95",
        class = "HUNTER",
        faction = "Horde",
      },
      [2] = {
        name = "Herrlu",
        guid = "Player-4477-03B5A5D5",
        class = "MAGE",
        faction = "Horde",
      },
    },
  },
  [40] = {
    name = "Greves",
    guid = "Player-4477-03AF1BAC",
    class = "HUNTER",
    faction = "Horde",
    description = "ZA GDKP Orga: Will loot behalten statt diss aber nicht zahlen.",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/972466166641619034",
    category = "gdkp",
    level = 3,
  },
  [41] = {
    name = "Gromgrul",
    guid = "Player-4477-03DE0567",
    class = "WARRIOR",
    faction = "Horde",
    description = "Plündert gern mal die Gildenbank",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/972958108475330570",
    category = "trade",
    level = 3,
  },
  [42] = {
    name = "Handwerkerer",
    guid = "Player-4477-04C72D08",
    class = "WARRIOR",
    faction = "Horde",
    description = "GDKP geht mit dem gesamten Pot 116k Gold offline",
    url = "https://discord.com/channels/613060619738021890/972036752283926578/1068898422259732531",
    category = "gdkp",
    level = 3,
    aliases = { "Handwerkerr", "Würfelbotqt" },
  },
  [43] = {
    name = "Happybeast",
    guid = "Player-4477-048E9F36",
    class = "HUNTER",
    faction = "Horde",
    description = "Deathrollbetrüger",
    url = "https://discord.com/channels/613060619738021890/972036752283926578/1038987893823242301",
    category = "trade",
    level = 3,
  },
  [44] = {
    name = "Hellsângel",
    guid = "Player-4477-04D9ADE0",
    class = "PALADIN",
    faction = "Horde",
    description = "GDKP - geht mit dem gesamten Pot offline / 23,5k Gold",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1078427424926220409",
    category = "gdkp",
    level = 3,
  },
  [45] = {
    name = "Hoibedere",
    guid = "Player-4477-03B9E56F",
    class = "PALADIN",
    faction = "Alliance",
    description = "Gönnt sich als ZA alle Splitter Greens und Hexerstecken",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/978261580716331038",
    category = "raid",
    level = 3,
  },
  [46] = {
    name = "Holyfansqt",
    guid = "Player-4477-04498816",
    class = "PALADIN",
    faction = "Horde",
    description = "Betrügt andere Spieler um 10.000 Gold Cut - erfindet dafür nicht existente Regeln. Der übrig gebliebene Pot wird dann durch 25 gerechnet und der entzogene Cut wird einfach einbehalten. Auch bereits auf anderen Servern bekannt.",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1136837983178338364",
    category = "gdkp",
    level = 3,
  },
  [47] = {
    name = "Hornylilcow",
    guid = "Player-4477-04489AB5",
    class = "DRUID",
    faction = "Horde",
    description = "GDKP PKD10er - Geht mit dem gesamten Pot offline und ninjat auch items. Zahlt dann einigen Leuten den CUT zurück um von der Blacklist entfernt zu werden.",
    url = "https://docs.google.com/spreadsheets/d/1IKAr8A4P0-LhkXqMxizvgYy1E2gph_00M_O0r3rDGkY/edit",
    category = "gdkp",
    level = 3,
  },
  [48] = {
    description = "Scammt Endboss-Tokens / Behält sich den Loot im 25er egal ob jemand Prio darauf hatte oder nicht",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1102594033651564546",
    category = "raid",
    level = 3,
    players = {
      [1] = {
        name = "Infernâ",
        guid = "Player-4477-03B07A37",
        class = "DRUID",
        faction = "Horde",
      },
      [2] = {
        name = "Rosabienchen",
        guid = "Player-4477-03B074F6",
        class = "PALADIN",
        faction = "Horde",
      },
    },
  },
  [49] = {
    name = "Javoor",
    guid = "Player-4477-035A99C2",
    class = "DRUID",
    faction = "Horde",
    description = "Rollt gern ohne zu Zahlen",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/997262970960760983",
    category = "trade",
    level = 3,
  },
  [50] = {
    name = "Jenpai",
    guid = "Player-4477-03B2BFD8",
    class = "PALADIN",
    faction = "Horde",
    description = "Booster kickt Leute / geht Offline nach Bezahlung ohne Leistung",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/983417620437032970",
    category = "dungeon",
    level = 3,
  },
  [51] = {
    name = "Jensha",
    guid = "Player-4477-03FF5FEF",
    class = "MAGE",
    faction = "Horde",
    description = "Booster kickt Leute / geht Offline nach Bezahlung ohne Leistung",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/980863146111672370",
    category = "dungeon",
    level = 3,
  },
  [52] = {
    name = "Jondeepfreez",
    guid = "Player-4477-0464DE81",
    class = "MAGE",
    faction = "Horde",
    description = "Scammt in 5er Gruppen alle Items, geht und dann offline ohne die Splitter zu verteilen / zudem gibt er es auch per whisper zu",
    url = "https://discord.com/channels/613060619738021890/1029711210918191154/1087391435378143393",
    category = "dungeon",
    level = 3,
  },
  [53] = {
    name = "Kartoffeljoe",
    guid = "Player-4477-04487832",
    class = "PALADIN",
    faction = "Horde",
    description = "Öffnet 10er Raid mit MS-OS und verteilt dann fair errollte Items lieber an seine Gildenkollegen.",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1155493204418908272",
    category = "raid",
    level = 3,
  },
  [54] = {
    name = "Keldradruîde",
    guid = "Player-4477-044F170E",
    class = "DRUID",
    faction = "Horde",
    description = "Ulduar 10er / Kickt Spieler aus Frust kurz bevor der Boss down ist, damit diese nicht looten können. Vorsicht Scam	",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1166847508794839171",
    category = "raid",
    level = 3,
  },
  [55] = {
    name = "Kessler",
    guid = "Player-4477-04BABDB4",
    class = "DRUID",
    faction = "Horde",
    description = "Hält sich nicht an Prio-Listen und verteilt Items nach seinem Ermessen",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1055966524466274315",
    category = "raid",
    level = 3,
  },
  [56] = {
    name = "Kisxí",
    guid = "Player-4477-03B293E1",
    class = "PALADIN",
    faction = "Horde",
    description = "Erhält in Naxx10 irrtümlicherweise ein falsches Item welches ihm nicht zusteht, zündet Ruhestein und geht offline. Endbossitem Naxx: Torch",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1096600239164629114",
    category = "raid",
    level = 3,
  },
  [57] = {
    name = "Kreiszahl",
    guid = "Player-4477-03346F2C",
    class = "SHAMAN",
    faction = "Horde",
    description = "Siehe auch Spieler Pi / Versucht Cut zu scammen (gesamt 8k)",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1101288642649014374",
    category = "gdkp",
    level = 3,
  },
  [58] = {
    name = "Kugit",
    guid = "Player-4477-044F88CE",
    class = "HUNTER",
    faction = "Horde",
    description = "Erstellt NaxxPUG ohne Lootsystem und tradet Endbossloot an jemanden der nicht per Roll gewonnen hat",
    url = "https://discord.com/channels/613060619738021890/972036752283926578/1068688364716445807",
    category = "raid",
    level = 3,
  },
  [59] = {
    name = "Lachman",
    guid = "Player-4477-03EC6CF8",
    class = "WARRIOR",
    faction = "Horde",
    description = "Ninjat Items als Plündermeister",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/980200744185630781",
    category = "raid",
    level = 3,
  },
  [60] = {
    name = "Laredai",
    guid = "Player-4477-04BDD735",
    class = "PRIEST",
    faction = "Horde",
    description = "Lockt Betrayer aber gibt diese Info nicht an nachträglich in den Raid geladene Spieler weiter. Nachzügler setzt Betrayer auf Prio1 - erhält trotzdem keine Info / Betrayer ist droppt und wurde an Stahlklang-Gildenmember vergeben",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1057476414938546236",
    category = "raid",
    level = 3,
  },
  [61] = {
    name = "Lebkuchen",
    guid = "Player-4477-040379B4",
    class = "PALADIN",
    faction = "Alliance",
    description = "Locken mitten im ZA Prio Run Items",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/976518445510328401",
    category = "raid",
    level = 3,
  },
  [62] = {
    name = "Listers",
    guid = "Player-4477-03B198AD",
    class = "MAGE",
    faction = "Horde",
    description = "Booster kickt Leute / geht Offline nach Bezahlung ohne Leistung",
    url = "https://docs.google.com/spreadsheets/d/1IKAr8A4P0-LhkXqMxizvgYy1E2gph_00M_O0r3rDGkY",
    category = "dungeon",
    level = 3,
  },
  [63] = {
    name = "Lookìí",
    guid = "Player-4477-04BC3E60",
    class = "DRUID",
    faction = "Horde",
    description = "Verrollt Items nicht - zieht sie einfach an / lässt Items im Raid ablaufen - sodass kein verrollen mehr möglich ist / DC-Tag: Lookii#9050",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1068320436456259654",
    category = "raid",
    level = 3,
  },
  [64] = {
    name = "Lovetap",
    guid = "Player-4477-03BE372B",
    class = "SHAMAN",
    faction = "Horde",
    description = "Antisemit",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/982000954377396244",
    category = "harassment",
    level = 3,
  },
  [65] = {
    name = "Lyzak",
    guid = "Player-4477-048F3883",
    class = "PALADIN",
    faction = "Horde",
    description = "Kassiert 2k Gold als Eintrittsgeld für 25Prio-Run + scamt dann Prio1 Item Turning Tide / Empfänger des Items: Lorani - gleiche Gilde / Discord-Tag: EinfachCanTV#8479",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1066022134540931243",
    category = "raid",
    level = 3,
  },
  [66] = {
    description = "Betrügt andere Spieler um 10.000 Gold Cut - erfindet dafür nicht existente Regeln. Der übrig gebliebene Pot wird dann durch 25 gerechnet und der entzogene Cut wird einfach einbehalten. Auch bereits auf anderen Servern bekannt.",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1136837983178338364",
    category = "gdkp",
    level = 3,
    players = {
      [1] = {
        name = "Maximabi",
        guid = "Player-4477-04D08212",
        class = "MAGE",
        faction = "Horde",
      },
      [2] = {
        name = "Maximqt",
        guid = "Player-4477-047B336C",
        class = "DEATHKNIGHT",
        faction = "Horde",
      },
    },
  },
  [67] = {
    name = "Mczwuggi",
    guid = "Player-4477-044CA3A0",
    class = "WARRIOR",
    faction = "Horde",
    description = "GDKP geht mit dem gesamten Pot offline.",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1045384854042324992",
    category = "gdkp",
    level = 3,
  },
  [68] = {
    name = "Melthise",
    guid = "Player-4477-04434888",
    class = "MAGE",
    faction = "Horde",
    description = "Booster kickt Leute / geht Offline nach Bezahlung ohne Leistung",
    url = "https://docs.google.com/spreadsheets/d/1IKAr8A4P0-LhkXqMxizvgYy1E2gph_00M_O0r3rDGkY/edit",
    category = "dungeon",
    level = 3,
  },
  [69] = {
    name = "Moonlooll",
    guid = "Player-4477-04BCE542",
    class = "DRUID",
    faction = "Horde",
    description = "PDOK 25 GDKP - geht mit dem gesamten Pot offline 108.000 Gold",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1160223729608036412",
    category = "gdkp",
    level = 3,
  },
  [70] = {
    name = "Morodil",
    guid = "Player-4477-0432F85F",
    class = "PRIEST",
    faction = "Horde",
    description = "Bekommt in PDOK 25 ein Item zugewiesen, welches ihm nicht gehörte und geht danach offline",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1158535753282900019",
    category = "raid",
    level = 3,
  },
  [71] = {
    name = "Mylifebelike",
    guid = "Player-4477-049A0545",
    class = "ROGUE",
    faction = "Horde",
    description = "Zahlt beim Crossgamble nicht - Wenn er verliert geht er offline (zweiter Char = Myshoxbelike)",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1124062214337351720",
    category = "trade",
    level = 3,
  },
  [72] = {
    name = "Mâxîmûs",
    guid = "Player-4477-03B07A08",
    class = "WARRIOR",
    faction = "Horde",
    description = "ICC25 NHC - Geht mit dem gesamten Loot vor Verteilung offline (alle Items bis Festergut)",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1168652939351576606",
    category = "gdkp",
    level = 3,
  },
  [73] = {
    name = "Nakrotss",
    guid = "Player-4477-0419454D",
    class = "MAGE",
    faction = "Horde",
    description = "Booster kickt Leute / geht Offline nach Bezahlung ohne Leistung",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/987944740307759145",
    category = "dungeon",
    level = 3,
  },
  [74] = {
    name = "Palaslust",
    guid = "Player-4477-04D8C118",
    class = "PALADIN",
    faction = "Horde",
    description = "Prio 1 Run: Der RL teilt das Sartharion-3D-Mount dem falschen Spieler zu und dieser geht instant offline. ",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1071209778996580422",
    category = "raid",
    level = 3,
  },
  [75] = {
    name = "Patapeng",
    guid = "Player-4477-022E45F0",
    class = "MAGE",
    faction = "Horde",
    description = "Ninjat ein Item vom Endboss in Naxx25, gibt es zu und sagt er schickt 1k Gold compensation / schickt dann ein Wurmfleisch mit 1k Gold Nachnahme",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1092478052723867798",
    category = "raid",
    level = 3,
    aliases = { "Bakery" },
  },
  [76] = {
    name = "Pi",
    guid = "Player-4477-00CDA567",
    class = "PRIEST",
    faction = "Horde",
    description = "Zahlt Cut per Post nicht aus - lässt sich dann 3x bitten - erst nachdem der BL Eintrag erstellt wurde zahlt er die Hälfte des unterschlagenen Goldes. Konversation mit dem Typen auf der BL eher schwierig.",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1101288642649014374",
    category = "gdkp",
    level = 3,
  },
  [77] = {
    name = "Pyrobot",
    guid = "Player-4477-05164551",
    class = "MAGE",
    faction = "Horde",
    description = "Kassiert die Mats und das Gold um Items herzustellen und geht dann offline. Spieler wurde auch ingame reportet und kassierte lt. Blizzard einen Accountbann.",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1166058411662774273",
    category = "trade",
    level = 3,
  },
  [78] = {
    name = "Razíèl",
    guid = "Player-4477-04D84FE0",
    class = "DEATHKNIGHT",
    faction = "Horde",
    description = "PDK 25 GDKP - Geht mit dem gesamten Pot 69.000 Gold offline",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1159211034960220221",
    category = "gdkp",
    level = 3,
    aliases = { "Zeirø" },
  },
  [79] = {
    name = "Reorxdruid",
    guid = "Player-4477-04AAE38E",
    class = "DRUID",
    faction = "Horde",
    description = "Bekommt in PDOK10 ein Item falsch getradet und scamt dieses dann indem er es nicht herausgibt. Vorsicht vor diesem Spieler - scamt, sobald er die Chance dazu bekommt",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1165072851854962788",
    category = "raid",
    level = 3,
  },
  [80] = {
    description = "Zorodan (RL) hat PM vergessen, called passen, würfelt trotzdem und verschwindet zusammen mit Rhoran nachdem beide items per need gewonnen haben.",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1097453560188375122",
    category = "raid",
    level = 3,
    players = {
      [1] = {
        name = "Rhoran",
        guid = "Player-4477-04AB949A",
        class = "DEATHKNIGHT",
        faction = "Horde",
      },
      [2] = {
        name = "Zorodan",
        guid = "Player-4477-0381BCBA",
        class = "PALADIN",
        faction = "Horde",
      },
    },
  },
  [81] = {
    name = "Risani",
    guid = "Player-4477-0515E3AC",
    class = "PALADIN",
    faction = "Horde",
    description = "VORSICHT BEKANNTER GDKP SCAMMER!!! GDKP Pot von 91.800g kam zusammen und die Aussage er kickt jetzt leacher. Nachdem er einige gekickt hatte hat er selbst die gruppe verlassen ohne den Cut zu verteilen.",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1151611466563211376",
    category = "gdkp",
    level = 3,
    aliases = { "Destrolayz", "Zixana", "Allfredo" },
  },
  [82] = {
    name = "Risani",
    guid = "Player-4477-0515E3AC",
    class = "PALADIN",
    faction = "Horde",
    description = "VORSICHT BEKANNTER GDKP SCAMMER!!! Öffnet PDK25 GDKP und verschwindet dann mit 106k Gold offline nachdem er fuck you in den Chat schreibt",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1149064882738704444",
    category = "gdkp",
    level = 3,
    aliases = { "Destrolayz", "Zixana", "Allfredo" },
  },
  [83] = {
    name = "Sandmanncs",
    guid = "Player-4477-0320C13C",
    class = "MAGE",
    faction = "Horde",
    description = "Schließt Raider willkürlich vom Cut aus DC-Tag: Sandmann#7274",
    url = "https://docs.google.com/spreadsheets/d/1IKAr8A4P0-LhkXqMxizvgYy1E2gph_00M_O0r3rDGkY/edit",
    category = "gdkp",
    level = 3,
  },
  [84] = {
    name = "Saucenbinder",
    guid = "Player-4477-045558EF",
    class = "WARLOCK",
    faction = "Horde",
    description = "Plündert gern mal die Gildenbank",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1035874189783093248",
    category = "trade",
    level = 3,
  },
  [85] = {
    name = "Shaynd",
    guid = "Player-4477-050C1E7B",
    class = "WARRIOR",
    faction = "Horde",
    description = "ICC 25 NHC - Scamt Items indem er sie aus eigenem Ermessen verteilt (vorsicht - wenn ihr den höchsten Roll auf MS habt gehört das Item nicht unbedingt euch)",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1165754689032294400",
    category = "raid",
    level = 3,
  },
  [86] = {
    name = "Spansau",
    guid = "Player-4477-044801AB",
    class = "PALADIN",
    faction = "Horde",
    description = "Unterschlägt Items in Raids",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1009144231627079750",
    category = "raid",
    level = 3,
  },
  [87] = {
    name = "Spansau",
    guid = "Player-4477-044801AB",
    class = "PALADIN",
    faction = "Horde",
    description = "Betrüger - klaut Juwe Mats",
    url = "https://discord.com/channels/613060619738021890/972036752283926578/1122530271149965423",
    category = "trade",
    level = 3,
  },
  [88] = {
    name = "Spansau",
    guid = "Player-4477-044801AB",
    class = "PALADIN",
    faction = "Horde",
    description = "Schleicht sich in Eventbossgruppen ein und beschwört nicht",
    url = "https://discord.com/channels/613060619738021890/1029711210918191154/1033409599526682706",
    category = "dungeon",
    level = 3,
  },
  [89] = {
    name = "Strongscam",
    guid = "Player-4477-04AC4DB2",
    class = "WARRIOR",
    faction = "Horde",
    description = "Ändert im Priorun die Prios ab nachdem die Liste veröffentlich wurde (wieder hidden und reopen). Gilde steht dahinter ... komplett meiden den Verein.",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1038474981937528913",
    category = "raid",
    level = 3,
    aliases = { "Strønghøld" },
  },
  [90] = {
    name = "Stysthy",
    guid = "Player-4477-04EE3626",
    class = "DEATHKNIGHT",
    faction = "Horde",
    description = "Würfelt 2x für ein Item bei Malygos. Der RL übersieht das und weist ihm ungerechtfertigt ein Item zu. Danach verlässt er direkt den Raid. Wurde wegen diesem Vorfall aus der Gilde gekickt.",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1073981448580763658",
    category = "raid",
    level = 3,
  },
  [91] = {
    name = "Sunnytätär",
    guid = "Player-4477-039FE91B",
    class = "PRIEST",
    faction = "Horde",
    description = "Locken mitten im ZA Prio Run Items",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/976518445510328401",
    category = "raid",
    level = 3,
  },
  [92] = {
    name = "Telrah",
    guid = "Player-4477-04FE21BB",
    class = "MAGE",
    faction = "Horde",
    description = "ICC 25 nhc - Scamt ein Item indem er es aus eigenem Ermessen verteilt (vorsicht - wenn ihr den höchsten Roll auf MS habt gehört das Item nicht unbedingt euch)",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1166431128937312397",
    category = "raid",
    level = 3,
    aliases = { "Zanrelot" },
  },
  [93] = {
    name = "Tialen",
    guid = "Player-4477-04829ADC",
    class = "PALADIN",
    faction = "Horde",
    description = "Lockt Betrayer aber gibt diese Info nicht an nachträglich in den Raid geladene Spieler weiter. Nachzügler setzt Betrayer auf Prio1 - erhält trotzdem keine Info / Betrayer ist droppt und wurde an Stahlklang-Gildenmember vergeben",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1057476414938546236",
    category = "raid",
    level = 3,
    aliases = { "Thilon" },
  },
  [94] = {
    name = "Totemholiker",
    guid = "Player-4477-04B3673D",
    class = "SHAMAN",
    faction = "Horde",
    description = "Zahlt beim Crossgamble nicht - Wenn er verliert geht er offline (zweiter Char = Mylifebelike)",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1124062214337351720",
    category = "trade",
    level = 3,
    aliases = { "Myshoxbelike" },
  },
  [95] = {
    name = "Toxiic",
    guid = "Player-4477-03891F47",
    class = "WARLOCK",
    faction = "Horde",
    description = "Geht als Buyer mit und hat kein Gold - beleidigt dann nach Aufforderung zu kaufen.",
    url = "https://discord.com/channels/613060619738021890/972036752283926578/989965855112716409",
    category = "gdkp",
    level = 3,
  },
  [96] = {
    name = "Trytofly",
    guid = "Player-4477-03BDE4D2",
    class = "PALADIN",
    faction = "Horde",
    description = "Scammer! Klaut Craftmats",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/962628189962768405",
    category = "trade",
    level = 3,
  },
  [97] = {
    name = "Ukrgul",
    guid = "Player-4477-02D6082C",
    class = "MAGE",
    faction = "Horde",
    description = "Booster kickt Leute / geht Offline nach Bezahlung ohne Leistung",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/982727605914599557",
    category = "dungeon",
    level = 3,
  },
  [98] = {
    name = "Ultraboost",
    guid = "Player-4477-04445671",
    class = "SHAMAN",
    faction = "Horde",
    description = "Zahlt falschen Cut aus und beleidigt auf Nachfrage",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/967767521560240169",
    category = "gdkp",
    level = 3,
  },
  [99] = {
    name = "Unheiligdk",
    guid = "Player-4477-050460AC",
    class = "DEATHKNIGHT",
    faction = "Horde",
    description = "PDOK 10er GDKP / hat das Endpaket (Tiefenkristalle, Kugel des Kreuzfahrers, Rezepte) an einen Spieler verschenkt, anstatt es rechtmäßig an den Höchstbieter zu traden",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1158753421491454053",
    category = "gdkp",
    level = 3,
  },
  [100] = {
    name = "Unikuhm",
    guid = "Player-4477-04BB8BEB",
    class = "DRUID",
    faction = "Horde",
    description = "Steckt sich bei Ony25 die Tasche und die Gems ein und betitelt das als Orga Cut (Raid war jedoch kein Gdkp)",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1131005690475970560",
    category = "raid",
    level = 3,
  },
  [101] = {
    name = "Valentinmlk",
    guid = "Player-4477-02D60832",
    class = "MAGE",
    faction = "Horde",
    description = "Booster kickt Leute / geht Offline nach Bezahlung ohne Leistung",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/982727605914599557",
    category = "dungeon",
    level = 3,
  },
  [102] = {
    name = "Vejrekaiah",
    guid = "Player-4477-0310DAC1",
    class = "MAGE",
    faction = "Horde",
    description = "GDKP geht mit dem gesamten Pot offline / 108.750 Gold",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1069276743430570035",
    category = "gdkp",
    level = 3,
  },
  [103] = {
    name = "Vibechekk",
    guid = "Player-4477-03C0FD2D",
    class = "SHAMAN",
    faction = "Horde",
    description = "Lockt rnd Items ohne Vorabinfo",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/982634146717401138",
    category = "raid",
    level = 3,
  },
  [104] = {
    name = "Vitalyv",
    guid = "Player-4477-043BDE5B",
    class = "PALADIN",
    faction = "Horde",
    description = "Würfelt auf alles Bedarf und versucht die Items dann gegen Gold an die Spieler zu verkaufen die Need haben.",
    url = "https://discord.com/channels/613060619738021890/1029711210918191154/1080245642896613417",
    category = "trade",
    level = 3,
  },
  [105] = {
    name = "Vullahi",
    guid = "Player-4477-0401E55B",
    class = "WARRIOR",
    faction = "Horde",
    description = "PDK25 / MS>OS / lockt nachträglich Items und steckt sich diese in die eigene Tasche oder Freunden zu",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1163290279785271477",
    category = "raid",
    level = 3,
    aliases = { "Saido" },
  },
  [106] = {
    name = "Wandan",
    guid = "Player-4477-041935AD",
    class = "MAGE",
    faction = "Horde",
    description = "Booster kickt Leute / geht Offline nach Bezahlung ohne Leistung",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/984875343175352361",
    category = "dungeon",
    level = 3,
  },
  [107] = {
    name = "Weedneyhustn",
    guid = "Player-4477-040DE346",
    class = "WARRIOR",
    faction = "Horde",
    description = "Geht als Buyer mit und hat kein Gold - beleidigt dann nach Aufforderung zu kaufen.",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/989901518922743868",
    category = "gdkp",
    level = 3,
    aliases = { "Rôôz" },
  },
  [108] = {
    name = "Weezyamk",
    guid = "Player-4477-050FE4E0",
    class = "DEATHKNIGHT",
    faction = "Horde",
    description = "PDK25 GDKP - Öffnet Raids, bietet aber selbst nichtmal Minbid auf ein Upgrade - Item wird nicht gedisst sondern nach dem Raid einfach angezogen",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1156577725637726359",
    category = "gdkp",
    level = 3,
  },
  [109] = {
    name = "Wonderholy",
    guid = "Player-4477-050BEB47",
    class = "PRIEST",
    faction = "Horde",
    description = "PDOK 25 GDKP - geht mit dem gesamten Pot 82444 Gold offline (kommuniziert sehr wahrscheinlich über Google Übersetzer)	",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1159919188580778045",
    category = "gdkp",
    level = 3,
  },
  [110] = {
    name = "Wortalmombat",
    guid = "Player-4477-04554B62",
    class = "WARRIOR",
    faction = "Horde",
    description = "Beleidigt aufs übelste den Gildenlead nach Kick (Homophob diesdas)",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1035617814171623474",
    category = "harassment",
    level = 3,
  },
  [111] = {
    name = "Wounds",
    guid = "Player-4477-03B82274",
    class = "WARRIOR",
    faction = "Horde",
    description = "GDKP 25 Ulduar: Verrollt nicht alle Items - Kickt als sich ein paar beschweren einfach alle Spieler aus dem Raid",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/963749339778412594",
    category = "gdkp",
    level = 3,
  },
  [112] = {
    name = "Xervaz",
    guid = "Player-4477-04A45C8A",
    class = "DEATHKNIGHT",
    faction = "Horde",
    description = "Würfelt gerne ohne zu zahlen / streitet alles ab / trägt nicht dazu bei den Fall aufzulösen / 100k",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1166626365496373328",
    category = "trade",
    level = 3,
  },
  [113] = {
    name = "Yaxrail",
    guid = "Player-4477-04D02AC4",
    class = "WARRIOR",
    faction = "Horde",
    description = "PvP Main Warrior Yaxrail macht Ulduar10 GDKP auf und täuscht Lags und Disconnects vor, nachdem Thorim seinen Runestone nicht dropt.",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1103120025340104785",
    category = "raid",
    level = 3,
  },
  [114] = {
    name = "Zayr",
    guid = "Player-4477-0476BF51",
    class = "PALADIN",
    faction = "Horde",
    description = "Lockt sich in Naxx25iger die Torch, als diese nicht droppt ninjat er sich einfach Turning Tide und geht mit Ansage offline	",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1100951022094008360",
    category = "raid",
    level = 3,
  },
  [115] = {
    name = "Zenjen",
    guid = "Player-4477-04DC35A4",
    class = "SHAMAN",
    faction = "Horde",
    description = "Kickt ohne Grund Leute aus ner angefangenen PDOK 25er ID ohne Loot zu verrollen.",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1124432246317776946",
    category = "raid",
    level = 3,
    aliases = { "Vhelron" },
  },
  [116] = {
    name = "Zizmo",
    guid = "Player-4477-04A0BDED",
    class = "DEATHKNIGHT",
    faction = "Horde",
    description = "Scammed bei Deathrolls (rolled /2-2 und so faxen) der Frechfuchs",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/995699503577436180",
    category = "trade",
    level = 3,
  },
  [117] = {
    name = "Zugzugondeez",
    guid = "Player-4477-04D7B02C",
    class = "WARRIOR",
    faction = "Horde",
    description = "Needet den gesamten Loot und gewinnt dann auch noch den Schurken Token in HC++",
    url = "https://discord.com/channels/613060619738021890/1029711210918191154/1143307109568610314",
    category = "dungeon",
    level = 3,
  },
  [118] = {
    name = "Zullser",
    guid = "Player-4477-03B58DCD",
    class = "MAGE",
    faction = "Horde",
    description = "Booster kickt Leute / geht Offline nach Bezahlung ohne Leistung",
    url = "https://docs.google.com/spreadsheets/d/1IKAr8A4P0-LhkXqMxizvgYy1E2gph_00M_O0r3rDGkY/edit",
    category = "dungeon",
    level = 3,
  },
  [119] = {
    name = "Zumbleistift",
    guid = "Player-4477-03B14913",
    class = "DRUID",
    faction = "Horde",
    description = "Kickt gern ungerechtfertigt Spieler aus HC ++ Dungeons und klaut ihnen somit die ID",
    url = "https://discord.com/channels/613060619738021890/1029711210918191154/1123313827539329044",
    category = "dungeon",
    level = 3,
  },
  [120] = {
    name = "Rexs",
    guid = "Player-4477-042ED8BF",
    class = "PALADIN",
    faction = "Horde",
    description = "Archa 25 / Kickt 7-9 Spieler nachdem denen schon eine ID verpasst wurde / VORSICHT: ID-Scam",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1173967320058695711",
    category = "raid",
    level = 3,
  },
  [121] = {
    name = "Rostlocke",
    guid = "Player-4477-0528F759",
    class = "PALADIN",
    faction = "Horde",
    description = "Archa 25 / Rollt auf Items die ihm nicht zustehen - gewinnt und geht dann offline",
    url = "https://discord.com/channels/613060619738021890/915181409155563521/1175393128019144825",
    category = "raid",
    level = 3,
  },
};
